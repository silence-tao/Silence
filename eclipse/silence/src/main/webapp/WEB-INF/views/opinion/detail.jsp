					...
					<div class="col-lg-9 detail-box">
						...
						<div class="opinion-near">
							<p>上一篇：
								<c:choose>
									...
									<c:otherwise><a href="/silence/opinion/detail/${front.opinionId }">${front.title }</a></c:otherwise>
								</c:choose>
							</p>
							<p>下一篇：
								<c:choose>
									<c:when test="${empty after }"><a href="/silence/opinion/">回到分享生活</a></c:when>
									<c:otherwise><a href="/silence/opinion/detail/${after.opinionId }">${after.title }</a></c:otherwise>
								</c:choose>	
							</p>
						</div>
						<div class="comment-box">
							...
							<div class="comment-publish">
								<textarea name="" id="commnet-text" data-sign="${opinionView.opinionSign }"></textarea>
								<input type="button" name="" id="" onclick="submitComment('${opinionView.opinionSign }')" value="发表" />
							</div>
